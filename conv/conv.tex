...
\only<2->{
\begin{displaymath}
C_{bo}(i, j) = \sum^{k=0}_{l-1} \sum^{x = 0}_{m-1} \sum^{y = 0}_{n-1} A_{bk}(x + i - \frac{m}{2}, y + j - \frac{n}{2}) F_{ko}(x, y)
\end{displaymath}
\begin{itemize}
\item<3-> $k$ is the input channel.
\item<4-> $o$ is the output channel.
\item<5-> $l$ is the number of input channels.
\item<6-> $(m, n)$ is the size of the filters.
\item<7-> $b$ is the batch.
\end{itemize}
}
\end{overlayarea}
...
\only<2->{
\begin{displaymath}
O_k(i, j) = \max_{\substack{0 \leq x < m \\ 0 \leq y < n}} I_k(x + i - \frac{m}{2}, y + j - \frac{n}{2})
\end{displaymath}
\begin{itemize}
